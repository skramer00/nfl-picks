...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const preference of preferences) {
      ...
      const result = await resend.emails.send({ from: `Pretzel Quest <${fromEmail}>`, to: recipient.email, subject: `${remaining.length} Week ${week} ${remaining.length === 1 ? "pick is" : "picks are"} still open`, html: pickReminderHtml({ name: recipient.name || "there", week, count: remaining.length, kind, matchups: remaining.map((game) => `${game.away_team.name} at ${game.home_team.name}`) }), ...(deliveryTime ? { scheduledAt: deliveryTime } : {}) });
